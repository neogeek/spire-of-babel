...
const findConfigFile = (input, filename = '.eslintrc') => new Promise(resolve => {

    const localConfigPath = path.resolve(path.join(input, filename));

    fs.stat(localConfigPath, err => {

        if (err) {

            resolve(path.resolve(path.join(__dirname, '..', filename)));
