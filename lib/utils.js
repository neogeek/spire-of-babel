...
const findConfigFile = (input, filename) => new Promise(resolve => {

    const localConfigPath = path.resolve(path.join(input, filename || DEAFAULT_CONFIG_FILENAME));

    fs.stat(localConfigPath, err => {

        if (err) {

            resolve(path.resolve(path.join(__dirname, '..', filename || DEAFAULT_CONFIG_FILENAME)));
